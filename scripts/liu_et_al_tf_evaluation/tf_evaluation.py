...
# import click
import argparse # Using instead of click
import random
...
def main():
    ...
    # Commenting three lines below out, because of how I'm feeding in the data
    ...
    test_instances = pd.read_json(data_path, lines=True) # Reworked version of lines above, suited to dataset format
    print(f'Number of test instances: {len(test_instances)}')

    test_examples = []
    for row in test_instances.iterrows(): # Minor edits to work with Pandas df instead of list:
        for disambiguation in row[1]['disambiguations']:
            for template_id, (template, answer) in enumerate(templates.items()):
                prompt = f"Q: {row[1]['ambiguous_sentence']} {template} {disambiguation} True or False?\nA:"
                test_examples.append({
                    'example_id': row[1]['id'],
                    'ambiguous_sentence_key': row[1]['ambiguous_sentence_key'],
    ...
    results_df.to_json(f'TF_evaluation_data/{model_name}-{today}.jsonl', lines=True, orient='records')
